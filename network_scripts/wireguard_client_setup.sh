...
sudo ip netns exec nsCLIENT ifconfig ens1f0v0 7.0.0.1 netmask 255.255.255.0
sudo ip netns exec nsSERVER ifconfig ens1f0v1 7.0.0.2 netmask 255.255.255.0

sudo ip netns exec nsCLIENT sudo ip route add 2.0.0.0/24 via 6.0.0.2
sudo ip netns exec nsCLIENT sudo ip route add 1.0.0.0/24 dev ens1f0v0
sudo ip netns exec nsCLIENT sudo arp -s 1.0.0.1 fe:13:bc:b6:46:cd


sudo ip netns exec nsSERVER sudo ip route add 1.0.0.0/24 via 6.0.0.1
sudo ip netns exec nsSERVER sudo ip route add 2.0.0.0/24 dev ens1f0v1
sudo ip netns exec nsSERVER sudo arp -s 2.0.0.1 46:6b:95:10:ba:95
